...
export default {
  async fetch(request, env) {
    ...
    try {
      // Step 0: Test basic connectivity to IRIS
      console.log("Step 0: Testing basic IRIS connectivity...");
      const testUrl = "https://service.iris.edu/fdsnws/dataselect/1/version";
      ...
      results.steps.push({
        step: "IRIS Connectivity Test",
        success: testResponse.ok,
        status: testResponse.status,
        time: testTime,
        version: testText.trim()
      });
      
      console.log("IRIS connectivity test:", testResponse.status, testTime, "ms");
      
      // Step 1: Fetch from IRIS (30 minutes, starting 48 hours ago)
      console.log("Step 1: Fetching from IRIS...");
      
      const now = new Date();
      const dataStartTime = new Date(now.getTime() - 48 * 60 * 60 * 1000); // 48 hours ago
      const endTime = new Date(dataStartTime.getTime() + 30 * 60 * 1000); // 30 minutes forward
      ...
      const irisUrl = "https://service.iris.edu/fdsnws/dataselect/1/query?" +
                      ...
                      `&start=${formatTime(dataStartTime)}&end=${formatTime(endTime)}` +
                      "&format=miniseed";
      
      console.log("Requesting:", irisUrl);
      
      const fetchStart = Date.now();
      const response = await fetch(irisUrl, {
        headers: {
          'User-Agent': 'VolcanoAudio/1.0 (Educational Research Project)',
      ...
      await env.R2_BUCKET.put(r2Key, data, {
        customMetadata: {
          ...
          station: 'OBL',
          channel: 'HHZ',
          startTime: formatTime(dataStartTime),
      ...
      results.totalTime = Date.now() - testStartTime;
      results.summary = {
        irisFetch: "✅ PASS",
        r2Save: "✅ PASS",
